...
// rows 참조를 외부에서 갱신할 수 있도록 객체로 감싸기
const state = { rows: [] };

function attachAutoSave() {
  if (attachAutoSave._attached) return;
  attachAutoSave._attached = true;
  ...
  root.addEventListener('input', (e) => {
    ...
    if (el instanceof HTMLTextAreaElement || el instanceof HTMLInputElement) {
      state.rows[i][field] = el.value;
      saveBoard(state.rows);
    }
  });
...
function init() {
  ...
  safeOn(btnParse, 'click', () => {
    ...
    state.rows = mergeScriptsIntoBoard(state.rows, scripts);
    saveBoard(state.rows);
    render(state.rows);
    if (bulk) bulk.value = '';
  });
