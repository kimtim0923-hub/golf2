...
function buildSlideScript({ topic, userNotes }) {
  // --- Shorts(1분) 원고 생성 정책 (PROMPT UPGRADE) ---
  // 1) 출력 규칙(필수)
  // - 따옴표 금지: 시작/끝에 쌍따옴표 사용 금지
  // - 나레이션 전용: 번호/표/제목/설명문/구분표시 없이, “나레이션 텍스트만”
  // - 문단 나눔: 단계 사이에 한 줄 공백
  // - 핵심 강조: 벌타/숫자/기준은 **굵게**
  // - 리듬감 구어체: “결론부터 말하면/여기서 문제는/핵심은” 적극 사용
  // - 전문용어 최소화: 약어/영문/괄호 설명 금지
  // - CTA는 딱 1번, 마지막만
  //
  // 2) 주제 유형 자동 판단(3유형)
  // - 유형A: 단일 룰(골린이 일상 라운딩) 훅=창피/억울
  // - 유형B: 리스트형(골린이 일상 룰 여러 개)
  //   트리거: 숫자+가지, 총정리/정리/모음/핵심만, 자주 틀리는, 꼭 알아야 할
  // - 유형C: 리스트형(로컬룰/대회룰/레프리/위원회/MLR 등)
  //   트리거: 로컬룰, 대회룰, 레프리, MLR, 위원회, 특수 상황
  //
  // 3) 2026 개정 반영 우선
  // - ‘2026 개정 룰 파일’ 우선 검토 요구사항을 위해, 앱 내부에 2026 MLR 요약을 내장해둠.
  // - 개정 내용 없으면 현행 표준 룰 기반으로 작성.
  //
  // 4) 룰 번호는 내레이션에서 언급하지 않음(출처 슬라이드/JSON에만 포함)
  ...
  const makeOneMinuteManuscript = () => {
    ...
    const typeB = () => {
      // 리스트 개수: 제목/훅/메모에 "N가지"가 있으면 그 숫자를 사용(최대 6, 최소 3)
      const m = textAll.match(/(\d+)\s*가지/);
      const nRaw = m ? parseInt(m[1], 10) : 5;
      const n = Math.max(3, Math.min(6, Number.isFinite(nRaw) ? nRaw : 5));

      const open = `이거 모르면 오늘 라운딩에서 바로 벌타 먹습니다. ${is2026Summary ? `2026년 바뀐 포인트 ${n}가지,` : `아마추어가 자주 틀리는 포인트 ${n}가지,`} 지금 1분에 끝낼게요.`;

      // 주의: “2026 개정룰 총정리” 같은 요청은 실제 개정 항목 목록이 필요합니다.
      // 이 앱은 외부 문서를 자동 조회하지 않으므로, 확정 내용이 없으면 가짜 정보를 만들지 않기 위해
      // ‘확인 필요’ 형태로만 구성합니다.
      const MLR_2026 = [
        {
          name: '내부 아웃오브바운즈(티샷만 적용 옵션)',
          desc: '내부 OB를 “티잉구역에서 한 스트로크에만” 적용하도록 설정하는 선택지가 추가됐습니다.',
          tip: '티샷은 안전/난이도 때문에 막되, 그 이후 스트로크는 내부 OB 제한을 풀 수 있어요(위원회가 선택).'
        },
        {
          name: '박힌 볼 제한적 구제 확장(심판 확인 전제)',
          desc: '다른 플레이어 피치마크(수리 전)로 공이 지면 아래에 박힌 게 “심판이 확인”되면 무벌타 구제를 허용하는 옵션이 추가됐습니다.',
          tip: '생중계/심판 판정 가능한 대회에서만 쓰라고 지침돼요.'
        },
        {
          name: '볼 움직임을 몰랐던 채 잘못된 장소 플레이(비디오 발견)',
          desc: '플레이 당시 인지 못했는데, 나중에 영상으로 “내가 볼을 움직였다”는 증거가 나온 경우를 다루는 로컬룰 옵션이 추가됐습니다.',
          tip: '핵심은 “플레이 당시 몰랐다”는 전제와, 이후 절차를 어떻게 처리하느냐예요(주로 생중계 대회용).'
        },
        {
          name: '그린 근처 플레이 라인 구제(2+2) 범위 확장',
          desc: '그린 2클럽 이내 장해물 + 내 볼도 2클럽 이내일 때의 “플레이 라인 구제(2+2)”를 특정 유형의 수리 중인 지면에도 확장할 수 있게 됐습니다.',
          tip: '예: 마이크 제거 자국 등. 프린지/그린 주변에서 라인이 방해되면 적용 가능(위원회 채택 필요).'
        },
        {
          name: '전동차량 사용 금지 로컬룰 목적 설명 업데이트',
          desc: '카트 사용을 금지하는 로컬룰의 설명이 업데이트되어, 플레이어가 “합리적으로” 승인된 탑승이라고 믿은 경우 위원회가 승인할 여지를 명확히 했습니다.',
          tip: '대회 공지에 “어디 구간은 탑승 허용” 같은 예외가 있을 수 있어요.'
        },
        {
          name: '심하게 손상된 클럽 교체/수리(제한) + 부품 사용',
          desc: '클럽 교체를 “부러짐/심한 손상”으로 제한하고, 세트 연속성을 유지하는 교체만 허용하는 로컬룰이 정교화됐고, 소지 부품으로 수리도 허용되도록 업데이트됐습니다.',
          tip: '예: 5번 아이언이 부러졌다고 추가 드라이버로 바꾸는 건 제한될 수 있어요.'
        },
      ];

      const PREFERRED_LIES = {
        name: '프리퍼드 라이(구제구역 축소)',
        desc: '프리퍼드 라이 구제 구역이 “1클럽 길이”에서 “스코어카드 길이”로 축소되는 안내가 포함되어 있습니다.',
        tip: '겨울룰/프리퍼드 라이는 위원회가 채택했을 때만 적용돼요.'
      };

      const usePgaVariant = /PGA|프리퍼드\s*라이|preferred\s*lies/i.test(textAll);

      const items = is2026Summary
        ? (usePgaVariant
      ...
      // CTA는 대본 전체에서 딱 1번, 마지막에만 출력(반복 금지)
